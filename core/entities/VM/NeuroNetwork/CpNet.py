...
class CpNet(nn.Module):
    ...
    def __init__(self, is_already_trained: bool = False, *args, **kwargs):
        """
        Contractor for creating CpNet with defined count of layers.
            ...
            if not is_already_trained:
                self.__model__ = Sequential(
                    OrderedDict(
                        [
                            ('Input_layer',
                             Neuro_block((780, 780), input_dropout_rate=0.05, kernel_size=1, class_to_create='Linear')),

                            ('First_layer', Neuro_block((1, 256), input_dropout_rate=0.1, kernel_size=2, padding=1,
                                                        class_to_create='Conv2d')),
                            ('Second_layer', Neuro_block((256, 64), input_dropout_rate=0.11, kernel_size=3, padding=1,
                                                         class_to_create='Conv2d')),
                            ('Third_layer', Neuro_block((64, 4), input_dropout_rate=0.12, kernel_size=4, padding=1,
                                                        class_to_create='Conv2d')),
                            ('Fourth_layer', Neuro_block((4, 1), input_dropout_rate=0.14, kernel_size=5, padding=1,
                                                         class_to_create='Conv2d')),  # 1689 x 764
                            ...
                            ('Seventh_layer',
                             Neuro_block((100, 10), input_dropout_rate=0.17, class_to_create='Linear')),
    ...
    @staticmethod
    def save_model(save_path: str | PathLike, __model_name: str = 'network',
                   ext: Literal['.pth', '.pt', '.pwf'] = '.pth', __model__=None) -> None:
        """
        ...
        try:
            ...
            for epoch in epoch_count:
                ...
                for images, labels in train_data_loader:
                    ...
                    print(
                        f'Current train epoch - {epoch + 1} of {epoch_count}, Loss: {running_loss / len(train_data_loader):.4f}.')
            if after_train_save:
            ...
            for epoch in range(test_epochs_count):
                ...
                for images, labels in test_data_loader:
                    ...
                    for image in images:
                        ...
                        running_loss += loss.item()
                    print(
                        f'Current test epoch - {epoch + 1} of {epoch}, Loss: {running_loss / len(test_data_loader):.4f}.')
            if after_test_save:
        ...
        try:
            if self.__model__ is not None:
                ...
                if parameters is not None:
                    self.__optimizer__ = torch.optim.Adam(self.__model__.parameters(),
                                                          lr=learning_rate)  # TODO в будущем заменить на Nadam.
